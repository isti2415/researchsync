...
const EditPaper = () => {
    ...
    const handleSubmit = async (values) => {
        await setDoc(doc(db, "projects", organization.id, "papers", router.query.paper), values)
            .then(() => {
                toast("Successfully saved changes");
                router.push(`/projects/${organization.slug}/papers/${router.query.paper}`)
            })
            .catch((error) => {
    ...
    if (form.values && user && !form.values.addedBy?.id == user.id) {
        return (
            ...
            <div className="flex flex-col mt-8 gap-4">
                <form onSubmit={form.handleSubmit}>
                    <div className="flex justify-end items-center gap-4">
                        ...
                        <Button type="submit" disabled={uploading > 0 && uploading < 100}>Save</Button>
                    </div>
